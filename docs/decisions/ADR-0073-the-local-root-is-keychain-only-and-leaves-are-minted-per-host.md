...
- Status: Accepted; implemented as `WailoCertificateAuthority` + `CertificateAuthorityStore` in `daemon` and a `ProxyTls` seam in `:proxy`, surfaced by `wailo-cli proxy_ca` and by Studio's Settings (create, save again, replace, remove, plus the unlocked-host list). Verify with `cd studio && ./gradlew :proxy:test :daemon:test` (`ProxyDecryptionTest` drives a real TLS origin through the relay, unlocked and locked).
