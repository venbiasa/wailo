...
- Status: Accepted; implemented as `SystemProxyController` in `daemon`, a `ProxyChain` seam in `:proxy`, and `wailo-cli set_system_proxy`. Verify with `cd studio && ./gradlew :proxy:test :daemon:test`.
