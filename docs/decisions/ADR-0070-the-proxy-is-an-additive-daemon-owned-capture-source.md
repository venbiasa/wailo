...
- Consequences:
  - Every rule set had to become evaluable on the desktop. A device enforces its own rules from a pushed snapshot; a proxied exchange has no device, so `daemon`'s `HostProxyRules` re-implements ADR-0033's precedence against the same `HeadlessHost` registries. That is a deliberate second copy of the matching order — the SDK's runs inside a third-party app and can depend on nothing here — and the two must be changed together.
