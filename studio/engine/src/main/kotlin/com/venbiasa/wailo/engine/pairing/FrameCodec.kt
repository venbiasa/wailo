...
class FrameRejected(message: String) : Exception(message)

/**
 * Seals and opens the frames of one WiFi session (ADR-0039). The mirror of
 * `sdk-ios/Sources/WailoSDK/WailoFrameCodec.swift`, with the directions swapped.
 *
 ...
 *
 * AES-256-GCM rather than ChaCha20-Poly1305 so the device side can use the same primitive: `javax.crypto`
 * only offers ChaCha at Android API 28 and the SDK's floor is 24.
 */
class FrameCodec(
